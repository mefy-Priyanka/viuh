...
@Component({
  selector: 'app-fleetdetail',
  templateUrl: './fleetdetail.component.html',
  styleUrls: ['./fleetdetail.component.css']
})
    ...
    private router: Router, private companyService:
      CompanyService, private toastr: ToastrService,private userService: UserService,private SharedService: SharedService,) {
        ...
        this.SharedService.somedata.subscribe(data => {
          ...
          this.currentfleet=data.data;
          console.log(this.currentfleet);
          this.sharedservicedata()
...
sharedservicedata(){
  this.trucknumber=this.currentfleet.truck_number;
  this.check('trucknumber',0);
  this.truckcapacity=this.currentfleet.capacity;
  this.check('truckcapacity',0);
  this.unit=this.currentfleet.unit;
  this.check('unit',0);
  if(this.currentfleet.contractId){
    this.ownership=this.currentfleet.contractId._id
  }
  else{
    this.ownership=this.currentfleet.ownId._id
  }
  console.log(this.ownership)
  this.check('ownership',this.ownership)
}
...
console.log(this.maindata)
  ...
  submit() {
    ...
    Object.assign(this.maindata, { fleetId: this.currentfleet._id })

    // Object.assign(this.maindata, { ownership: this.ownership })

    console.log(this.maindata);
    
    this.companyService.fleetedit(this.maindata).subscribe(result => {
      console.log(result);
     
      this.toastr.success('Awesome!', 'fleet Updated successfully');
    },
      err => {
        console.log(err)
